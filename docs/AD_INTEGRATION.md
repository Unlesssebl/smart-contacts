...
4. **Скрипт первичной инициализации (`seed_ou_mapping.py`)**: В `services/api_gateway/scripts/seed_ou_mapping.py` реализована утилита первично наполнить `OU_MAPPING` преднастроенными предприятиями.

### Алгоритм сопоставления
1. **Динамическая загрузка**: Маппинг извлекается из БД (`system_settings` ключ `OU_MAPPING`) в виде JSON-объекта (ключ: `OU`, значение: Объект с полем `org` (название организации) или строка). Этот маппинг настраивается через панель администратора IT-оператором.
