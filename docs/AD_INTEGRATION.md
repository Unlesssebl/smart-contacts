...
1. **Динамическая загрузка**: Маппинг извлекается из БД (`system_settings` ключ `OU_MAPPING`) в виде JSON-объекта (ключ: `OU`, значение: Название организации). Этот маппинг настраивается через панель администратора IT-оператором.
