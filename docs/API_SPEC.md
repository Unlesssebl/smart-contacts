...
|----------|-----|-------------|----------|
| `q` | string | — | Нечёткий поиск (fuzzy) по `full_name`, `department`, `office_location` через `pg_trgm` |
...
**Ответ 200:** Схема идентична `GET /auth/me` (без полей `role`, `grace_period_left`).

**Примечание**: Для профилей с `is_protected = true` поле `internal_phone` скрыто для обычных пользователей (возвращается `null`). IT-Operator видит полностью.
