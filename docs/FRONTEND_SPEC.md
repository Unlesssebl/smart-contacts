...
223: ---
224: 
225: ## Синхронизация статусов присутствия
226: 
227: ### `usePresence` Hook
228: Фронтенд отслеживает статус активности пользователя с помощью React-хука `usePresence()`, который вызывается глобально (например, в корневом `App.tsx` / `main.tsx` при успешной авторизации).
229: - **Подключение**: Хук инициализирует WebSocket-соединение с `/ws/presence`, получая одноразовый JWT-токен.
230: - **Анализ бездействия**:
