...
3. **OU Маппинг (`<OuMappingPanel>`)** — управление сопоставлением OU Active Directory и предприятий:
   - Палитра из 12 гармоничных пресетов (`ORGANIZATION_COLOR_PALETTE`) + ручной ввод HEX-кода через поповер `<ColorSelect>`.
   - Автоматическое хэширование цвета для новых предприятий (`hashOrganizationColor`).
   - Массовое выделение предприятий (чекбоксы) и групповое назначение цвета («Выбрано предприятий: N» → «Назначить цвет»).
   - Предупреждение о ненайденных OU в авторитетном дереве AD.
