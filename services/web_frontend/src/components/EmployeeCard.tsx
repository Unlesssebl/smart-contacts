  ...
  ({ user, onClick }, ref) => {
    ...
    return (
      <div
        ref={ref}
        onClick={onClick}
        className={`relative group cursor-pointer p-6 bg-white border border-slate-200/90 shadow-[0_14px_34px_-28px_rgba(15,34,58,0.42)] rounded-[20px] hover:-translate-y-1 hover:border-[#9fb9d4] hover:shadow-[0_24px_42px_-28px_rgba(15,34,58,0.62)] transition-all duration-300 flex flex-col h-full overflow-hidden ${user.is_hidden ? 'opacity-60 grayscale-[0.2]' : ''}`}
      >
        <div
        ...
        {!isOwnProfile && (
          <button
            ...
            className="btn-secondary absolute top-4 right-4 opacity-0 group-hover:opacity-100 text-xs px-3 py-1.5 z-10"
          >
        ...
        <div className="relative flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0 pt-1">
            ...
            {user.job_title && user.job_title !== '[]' && (
              <p className="mt-1.5 truncate text-[14px] text-white/80 font-medium leading-snug">{user.job_title}</p>
            )}
